...
export function getMatchResult(
  match: {
    ...
    bForfeit: boolean;
    WinningTeam: number;
    Players: Array<{ PlayerID: string; LastTeam: number }>;
  },
  localPlayerId: string,
): string {
  if (match.bNoContest) return "No contest";
  if (match.bForfeit) return "Forfeit";
  if (match.WinningTeam === -1) return "Tie";

  const localPlayer = findLocalPlayer(match.Players, localPlayerId);
  if (!localPlayer) return "Unknown";
  return localPlayer.LastTeam === match.WinningTeam ? "Win" : "Loss";
}
